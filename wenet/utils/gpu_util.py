...
def get_gpu_compute_mode(gpu_id=0):
    ...
    return gpu_info.stdout.strip().split()[-1]
    ...
    while True:
        for i in range(torch.cuda.device_count()):
            try:
                ...
                if 'EXCLUSIVE' not in get_gpu_compute_mode(i).upper():
                    logging.warning('GPU-{} is not in exclusive-process mode.'
                        ' You can use \'nvidia-smi -c 3 -i {}\' to let this GPU'
                        ' work in exclusive mode.'.format(i, i))
                return device, i
            except RuntimeError:
